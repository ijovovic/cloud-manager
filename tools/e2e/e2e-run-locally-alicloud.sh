...
#
# KNOWN GAP (verify before relying on this):
#   The sim shoot builder (e2e/sim/shootBuilder.go) has provider cases only for
#   gcp/aws/azure/openstack and NO alicloud case. The Runtime builder
#   (e2e/lib/runtimeBuilder.go) does have alicloud region/zone/machine defaults, so the
#   Runtime CR is fine, but the generated Shoot will have an empty AliCloud
#   InfrastructureConfig/ControlPlaneConfig. "instance create -p alicloud" may hang or
#   produce an unusable shoot until an `alicloud` case is added to shootBuilder.go.
#
# Prerequisites:
